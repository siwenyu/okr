      ...
      draw(cfg, group) {
        ...
        group.addShape('text', {
          attrs: {
            ...
            x: 12 + nodeOrigin.x,
            y: 20 + nodeOrigin.y,
            text: `${name.length > 28 ? `${name.slice(0, 28)  }...` : name} qw`,
            fontSize: 12,
            opacity: 0.85,
            fill: '#000',
            cursor: 'pointer',
  ...
  G6.registerEdge(
    'flow-cubic',
    {
      ...
      getPath(points) {
        ...
        const path = [
          ['M', startPoint.x + 100, startPoint.y],
          ['L', (startPoint.x + endPoint.x) / 2 + 100, startPoint.y],
          ['L', (startPoint.x + endPoint.x) / 2 + 100, endPoint.y ],
...
const initGraph = (data) => {
  ...
  const tooltip = new G6.Tooltip({
    ...
    getContent: (e) => {
      ...
      for (const [i, element] of nodeName.entries()) {
        formatedNodeName = `${formatedNodeName}${element}`;
  ...
  graph = new G6.TreeGraph({
    ...
    ...defaultConfig,
    ...config,
    plugins: [tooltip, minimap, snapLine],
    direction: 'LR',
    isHorizontal: true,
  });
...
console.log(document.querySelector('#rect1'));
